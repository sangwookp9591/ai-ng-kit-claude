  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    try {
      ...
      if (isLastAttempt) {
        ...
        if (featureName) recordFailure(featureName, errorMessage, projectDir);
 ...
 */
export function withRetry<T>(fn: (...args: unknown[]) => Promise<T>, options: RetryOptions = {}): (...args: unknown[]) => Promise<RetryResult<T>> {
  return (...args: unknown[]) => retryWithBackoff(() => fn(...args), options);
}

/**
 * Calculate retry delay for display.
 */
export function getRetrySchedule(attempt: number, baseDelayMs: number = 1000): RetrySchedule {
  const delays: number[] = [];
