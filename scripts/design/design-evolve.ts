...
import type { DesignSystem, DesignBrief, DesignTokens } from './design-engine.js';
...
function mutateHexColor(hex: string, intensity: number): string {
  // Remove # prefix
...
function mutateTokens(tokens: DesignTokens, rate: number): DesignTokens {
  const mutated: DesignTokens = JSON.parse(JSON.stringify(tokens));
...
export function evolveDesign(
  ...
  let population: Array<{ name: string; system: DesignSystem }> = [];
 ...
 */
export function formatEvolution(result: EvolutionResult): string {
  const lines: string[] = [];
