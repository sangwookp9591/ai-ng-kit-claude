...
function ghExec(args: string, dir: string, timeoutMs = 15000): string {
  return execSync(`gh ${args}`, { cwd: dir, encoding: 'utf-8', timeout: timeoutMs }).trim();
}

function gitExec(args: string, dir: string, timeoutMs = 30000): string {
  return execSync(`git ${args}`, { cwd: dir, encoding: 'utf-8', timeout: timeoutMs }).trim();
}

function getPrInfo(prNumber: number, dir: string): PrInfo | null {
  try {
    const raw = ghExec(
      `pr view ${prNumber} --json reviewDecision,mergeable,baseRefName,headRefName,url,title`,
      dir,
    );
    ...
    try {
      ...
      const failing = checks.filter((c: { bucket: string }) => c.bucket === 'fail');
...
function runPreMergeChecks(prNumber: number, dir: string): LandStep {
...
function runMerge(prNumber: number, strategy: MergeStrategy, dryRun: boolean, dir: string): LandStep {
  const start = Date.now();
...
export async function executeLandPipeline(options: LandOptions): Promise<LandResult> {
  const {
    prNumber,
    feature,
  ...
  let deployUrl: string | null = canaryUrl ?? null;
  ...
  try {
    ...
    steps.push(deploy);
    if (deploy.details.url) deployUrl = deploy.details.url as string;
...
export function formatLandReport(result: LandResult, prNumber: number): string {
  const lines: string[] = [
    `## Deployment Report`,
    '',
    `**PR:** #${prNumber}`,
