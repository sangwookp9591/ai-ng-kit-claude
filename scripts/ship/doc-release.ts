 ...
 */
export const DOC_FILES: DocFileEntry[] = [
 ...
 */
export function findStaleDocs(changedFiles: string[], projectDir?: string): StaleDoc[] {
  ...
  for (const doc of DOC_FILES) {
    ...
    if (doc.file === 'README.md') {
      ...
      if (hasNewEndpoints || hasNewFeatures) reason = 'New features/endpoints may need documentation';
    }

    if (doc.file === 'ARCHITECTURE.md') {
      const hasStructureChange = changedFiles.some(f =>
        f.includes('scripts/') || f.includes('hooks-handlers/') || changedFiles.length > 10
      );
      if (hasStructureChange) reason = 'Structural changes may affect architecture docs';
    }

    if (doc.file === 'CLAUDE.md') {
      const hasAgentChange = changedFiles.some(f => f.includes('agents/') || f.includes('skills/'));
      if (hasAgentChange) reason = 'Agent/skill changes may need project context update';
    }
 ...
 */
export function getChangedFiles(baseBranch: string, projectDir?: string): string[] {
  const dir = projectDir || process.cwd();
  try {
    const raw = execSync(
 ...
 */
export function buildDocUpdatePrompt(staleDocs: StaleDoc[], changedFiles: string[]): string {
  if (staleDocs.length === 0) return 'No documentation updates needed.';
 ...
 */
export function formatDocReleaseSummary(staleDocs: StaleDoc[]): string {
  if (staleDocs.length === 0) return 'All docs up to date.';

  const lines: string[] = [`Doc Release: ${staleDocs.length} docs may need updates`];
  for (const doc of staleDocs) {
