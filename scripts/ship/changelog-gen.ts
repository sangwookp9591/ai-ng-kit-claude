 ...
 */
export function getCommitsSince(since?: string | null, projectDir?: string): CommitEntry[] {
 ...
 */
export function generateChangelog(version: string, commits: CommitEntry[]): string {
  const date = new Date().toISOString().slice(0, 10);
  const grouped: Record<string, Array<ParsedCommit & { hash: string }>> = {};
 ...
 */
export function prependChangelog(content: string, projectDir?: string): void {
  const dir = projectDir || process.cwd();
...
function getLastTag(dir: string): string | null {
  try {
