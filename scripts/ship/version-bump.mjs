 ...
 * @param {string} version - e.g. "1.2.3" or "v1.2.3"
 ...
 * @param {number} signals.filesChanged
 * @param {boolean} signals.hasBreaking - Breaking API changes
 * @param {boolean} signals.hasNewFeature - New functionality added
 * @param {boolean} signals.hasBugFix - Bug fixes
 * @returns {'major'|'minor'|'patch'}
 */
export function determineBumpType(signals) {
 ...
 * @returns {{ oldVersion: string, newVersion: string, bumpType: string }}
 */
export function bumpVersion(bumpType, projectDir) {
 ...
 * @returns {string}
 */
export function readVersion(projectDir) {
  const dir = projectDir || process.cwd();
  const versionFile = join(dir, 'VERSION');
  const pkgFile = join(dir, 'package.json');
