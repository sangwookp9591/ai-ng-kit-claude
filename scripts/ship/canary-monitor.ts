 ...
 */
export function runCanaryLoop(options: CanaryOptions): CanaryResult {
  const { url, feature, checks = 5, projectDir } = options;
  const results: CanaryCheckEntry[] = [];
...
export function formatCanaryResult(result: CanaryResult): string {
  const lines: string[] = [`Canary: ${result.passed ? 'HEALTHY' : 'ALERT'} (${result.checks.length} checks)`];
