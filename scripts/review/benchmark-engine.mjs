 ...
 * @property {number} domInteractive - DOM Interactive (ms)
 * @property {number} domComplete - DOM Complete (ms)
 * @property {number} fullLoad - Full Load (ms)
 * @property {number} totalRequests
 * @property {number} totalTransfer - Total transfer bytes
 * @property {number} jsBundle - JS bundle bytes
 * @property {number} cssBundle - CSS bundle bytes
 */

/**
 * Compare current metrics against baseline.
 * @param {PerfMetrics} current
 * @param {PerfMetrics} baseline
 * @returns {Array<{ metric: string, baseline: number, current: number, delta: string, status: string }>}
 */
export function compareMetrics(current, baseline) {
  ...
  for (const key of timingMetrics) {
    ...
    const pctChange = baseline[key] > 0 ? ((current[key] - baseline[key]) / baseline[key]) * 100 : 0;
    const absChange = current[key] - baseline[key];
 ...
 * @param {PerfMetrics} metrics
 * @returns {Array<{ metric: string, budget: number, actual: number, status: string }>}
 */
export function checkBudgets(metrics) {
  ...
  for (const [key, config] of Object.entries(BUDGETS)) {
    ...
    results.push({
      ...
      actual: metrics[key],
      unit: config.unit,
      status: metrics[key] <= config.budget ? 'PASS' : 'FAIL',
    });
  }
 ...
 * @param {Array} budgetResults - Output of checkBudgets()
 ...
 * @param {Array} comparison - Output of compareMetrics()
 * @param {Array} budgets - Output of checkBudgets()
 * @param {string} grade
 * @returns {string}
 */
