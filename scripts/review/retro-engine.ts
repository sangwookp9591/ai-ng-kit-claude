 ...
 */
export function gatherMetrics(window: string = '7d', projectDir?: string): GitMetrics {
  ...
  try {
    ...
    const authors: AuthorStats[] = shortlog.split('\n').filter(Boolean).map(line => {
      const match = line.trim().match(/^\s*(\d+)\s+(.+)$/);
      return match ? { commits: parseInt(match[1]), name: match[2] } : null;
    }).filter((a): a is AuthorStats => a !== null);

    // File hotspots
    let hotspots: FileHotspot[] = [];
    try {
    ...
    log.error(`Failed to gather metrics: ${(err as Error).message}`);
 ...
 */
export function detectSessions(commits: CommitInfo[]): WorkSession[] {
 ...
 */
export function classifyCommits(commits: CommitInfo[]): CommitTypes {
  const types: CommitTypes = { feat: 0, fix: 0, refactor: 0, test: 0, chore: 0, docs: 0, style: 0, ci: 0, other: 0 };
  for (const c of commits) {
    const match = c.message?.match(/^(\w+)/);
    const type = match?.[1]?.toLowerCase() as keyof CommitTypes | undefined;
 ...
 */
export function calculateFocusScore(hotspots: FileHotspot[]): FocusScore {
 ...
 */
export function generateRetro(window: string = '7d', projectDir?: string): string {
  ...
  for (const [type, count] of (Object.entries(commitTypes) as [string, number][]).filter(([, c]) => c > 0)) {
 ...
 */
export function saveRetroSnapshot(metrics: GitMetrics, projectDir?: string): void {
  const dir = projectDir || process.cwd();
