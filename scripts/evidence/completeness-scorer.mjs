 ...
 * @param {object} [evidence.tests] - { passed: number, total: number }
 * @param {object} [evidence.build] - { success: boolean }
 * @param {object} [evidence.lint] - { errors: number }
 * @param {object} [evidence.goals] - { conditions: Array<{ name: string, met: boolean }> }
 * @param {object} [evidence.edgeCases] - { handled: number }
...
export function calculateCompleteness(evidence) {
  ...
  if (evidence.goals?.conditions?.length > 0) {
  ...
  if (evidence.edgeCases?.handled > 0) score += 1;
 ...
 * @returns {'ACHIEVED' | 'COMPLETED BUT INCOMPLETE' | 'FAILED'}
 */
 ...
 * @param {number} score - 0-10
 * @param {object} evidence - Same shape as calculateCompleteness input
 ...
 *
 * @param {object} evidence
 * @returns {{ score: number, verdict: string, report: string }}
 */
