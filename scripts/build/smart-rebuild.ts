 ...
 */
export function needsRebuild(sources: string[], target: string): boolean {
  ...
  return sources.some((src: string): boolean => {
    try {
 ...
 */
export function staleTargets(pairs: SourceTargetPair[]): string[] {
  return pairs
