...
function getSharedDeps(): string[] {
  ...
  if (existsSync(RESOLVERS_DIR)) {
    const resolverFiles = readdirSync(RESOLVERS_DIR)
      ...
      .map((f: string) => join(RESOLVERS_DIR, f));
...
function main(): void {
  ...
  const skillDirs = readdirSync(SKILLS_DIR, { withFileTypes: true })
    ...
    .map((e) => e.name)
    .filter((name: string) => existsSync(join(SKILLS_DIR, name, 'SKILL.md.tmpl')));
  ...
  if (staleByMtime.length === 0) {
    ...
    try {
      ...
      const staleByContent: StaleFile[] = [];
...
function reportStale(staleFiles: StaleFile[]): void {
