...
try {
  ...
  const freshResult = execSync(`node ${join(__dirname, 'check-freshness.mjs')}`, {
    encoding: 'utf-8',
    cwd: join(__dirname, '..', '..'),
  });
  console.log(freshResult);

  const hasStale = freshResult.includes('STALE') || freshResult.includes('stale');
  ...
  if (force || hasStale) {
    ...
    const genResult = execSync(`node ${join(__dirname, 'gen-skill-docs.mjs')}`, {
      encoding: 'utf-8',
      cwd: join(__dirname, '..', '..'),
    });
