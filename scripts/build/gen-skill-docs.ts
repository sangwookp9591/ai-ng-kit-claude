...
function buildPlaceholders(): Placeholders {
  let version = '0.0.0';
  try {
 ...
 */
function detectTier(tmplContent: string): string {
 ...
 */
function resolvePlaceholders(content: string, placeholders: Placeholders): string {
 ...
 */
function discoverTemplates(): TemplateInfo[] {
...
function main(): void {
  ...
  const outDirIdx: number = args.indexOf('--out-dir');
  const outDir: string | null = outDirIdx !== -1 ? args[outDirIdx + 1] : null;
