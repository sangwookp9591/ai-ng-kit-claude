 ...
 */
export const SKILL_TOUCHFILES: Record<string, string[]> = {
 ...
 */
const GLOBAL_TOUCHFILES: string[] = [
  'scripts/core/',
  'hooks-handlers/',
  'aing.config.json',
];
 ...
 */
function matchesPattern(changedFile: string, pattern: string): boolean {
 ...
 */
export function affectedSkills(changedFiles: string[]): string[] {
  if (!changedFiles || changedFiles.length === 0) return [];

  // Check if any global touchfile was changed — if so, ALL skills affected
  const globalChanged = changedFiles.some((f: string) =>
    GLOBAL_TOUCHFILES.some((pattern: string) => matchesPattern(f, pattern))
  );
  ...
  for (const file of changedFiles) {
    for (const [skill, patterns] of Object.entries(SKILL_TOUCHFILES)) {
      if (patterns.some((pattern: string) => matchesPattern(file, pattern))) {
        affected.add(skill);
      }
    }
  }
