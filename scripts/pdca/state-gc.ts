...
function getArchiveDir(projectDir: string): string {
 ...
 */
function isZombie(feature: PdcaFeature, maxAgeDays: number): boolean {
 ...
 */
export function runGC(projectDir: string, options: GCOptions = {}): GCResult {
  ...
  const archiveFeatures: Record<string, PdcaFeature> = {};
  ...
  const archiveResult: WriteResult = writeState(archivePath, {
  ...
  const writeResult: WriteResult = writeState(statePath, state);
  if (!writeResult.ok) {
