 ...
 */
export function classifyDecision(decision: DecisionInput): DecisionClassification {
 ...
 */
function findApplicablePrinciple(decision: DecisionInput): DecisionPrinciple | null {
 ...
 */
export function buildAutoplanPipeline(context: AutoplanContext): AutoplanPipeline {
  const tiers = selectTiers(context.complexityLevel, {
    hasUI: context.hasUI,
    hasProductChange: context.hasProductChange,
  });

  const phases: AutoplanPhase[] = [];
  const decisions: AutoplanDecision[] = [];
 ...
 */
export function formatAutoplanProgress(phases: AutoplanPhase[], currentPhase: number): string {
  const lines: string[] = ['Autoplan Pipeline:'];
 ...
 */
export function formatDecisionsSummary(decisions: AutoplanDecision[]): string {
  if (decisions.length === 0) return 'No decisions required.';

  const lines: string[] = ['Autoplan Decisions:'];
