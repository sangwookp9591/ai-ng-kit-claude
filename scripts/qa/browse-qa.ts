...
import type { Result } from '../../browse/src/browse-wrapper.js';
...
export async function runBrowseQA(
  projectDir: string,
  url: string,
  outputDir: string,
): Promise<BrowseQAResult> {
  ...
  const screenshots: string[] = responsiveResult.ok ? responsiveResult.data : [];
...
export async function runFlowTest(
  projectDir: string,
  steps: Array<{ action: string; selector?: string; value?: string; expect?: string }>,
  outputDir: string,
): Promise<BrowseQAResult> {
  const browseResult = await createBrowse(projectDir);
  if (!browseResult.ok) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    try {
      ...
      issues.push({
        severity: 'critical',
        description: `Step ${i + 1} failed: ${(err as Error).message}`,
      });
    }
  }
