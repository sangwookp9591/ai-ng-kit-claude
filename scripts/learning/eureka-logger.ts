 ...
 */
export function logEureka(projectDir: string, entry: EurekaEntry): EurekaRecord {
  const dir = join(projectDir, EUREKA_DIR);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });

  const record: EurekaRecord = {
 ...
 */
export function getRecentEurekas(projectDir: string, limit: number = 5): EurekaRecord[] {
  const filePath = join(projectDir, EUREKA_FILE);
  if (!existsSync(filePath)) return [];

  const lines = readFileSync(filePath, 'utf8').trim().split('\n').filter(Boolean);
  return lines.slice(-limit).map((line: string): EurekaRecord | null => {
    try { return JSON.parse(line); }
    catch { return null; }
  }).filter(Boolean) as EurekaRecord[];
}
 ...
 */
export function getReusableEurekas(projectDir: string): EurekaRecord[] {
 ...
 */
export function formatEurekas(eurekas: EurekaRecord[]): string {
  if (eurekas.length === 0) return '';

  const lines: string[] = ['[aing Eureka Log]'];
