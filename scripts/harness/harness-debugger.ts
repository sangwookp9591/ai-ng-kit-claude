...
export function diagnoseHarness(feature: string, projectDir: string): DiagnosisResult {
...
function detectStalledAgents(
  ...
  if (!existsSync(workspaceDir)) return stalled;
...
function detectMissingArtifacts(config: HarnessConfig, projectDir: string): string[] {
  const missing: string[] = [];
  const workspaceDir = join(projectDir, '_workspace');

  for (const edge of config.dataFlow) {
    if (edge.source === 'input') continue;
    if (!edge.artifact) continue;
...
function detectBrokenLinks(config: HarnessConfig, _projectDir: string): DataFlowEdge[] {
  const broken: DataFlowEdge[] = [];
