...
export function validateHarness(projectDir: string): ValidationResult {
...
function validateStructure(config: HarnessConfig): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
...
function validateConnections(config: HarnessConfig): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
...
function detectCycle(node: string, edges: { source: string; target: string }[], visited: Set<string>, inStack: Set<string>): boolean {
  ...
  for (const edge of edges) {
    if (edge.source === node && edge.target !== 'output' && edge.target !== 'integrator') {
      if (detectCycle(edge.target, edges, visited, inStack)) return true;
    }
  }
...
function validateQuality(config: HarnessConfig): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
...
function validateTeamSizing(config: HarnessConfig): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
...
function issue(severity: Severity, category: string, message: string, file?: string): ValidationIssue {
...
export function formatValidation(result: ValidationResult): string {
  ...
  for (const [severity, items] of Object.entries(grouped)) {
    ...
    for (const item of items as ValidationIssue[]) {
