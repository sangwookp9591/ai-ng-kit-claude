...
function galleryDir(projectDir: string): string {
...
const BUILTIN_PATTERNS: GalleryEntry[] = [
  {
...
export function initGallery(projectDir: string): void {
  const dir = galleryDir(projectDir);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });

  const existing = readStateOrDefault(indexPath(projectDir), null) as GalleryEntry[] | null;
  if (!existing) {
...
export function getPatterns(projectDir: string): GalleryEntry[] {
  initGallery(projectDir);
  return readStateOrDefault(indexPath(projectDir), BUILTIN_PATTERNS) as GalleryEntry[];
}

export function registerPattern(entry: GalleryEntry, projectDir: string): void {
...
export function searchPatterns(query: string, projectDir: string): GalleryEntry[] {
  const patterns = getPatterns(projectDir);
...
function matchScore(entry: GalleryEntry, query: string): number {
  let score = 0;
  ...
  // Domain match
  if (entry.domain.toLowerCase().includes(query)) score += 8;
  ...
  // Description match
  if (entry.description.toLowerCase().includes(query)) score += 2;
...
export function formatGallery(entries: GalleryEntry[]): string {
  if (!entries.length) return '  검색 결과 없음.';
