...
function loadStore(projectDir: string): PatternStore {
  return readStateOrDefault(statePath(projectDir), { patterns: [] }) as PatternStore;
}
 ...
 */
function findPattern(store: PatternStore, type: LearnablePattern['type'], pattern: string): LearnablePattern | undefined {
 ...
 */
export function recordPatternUse(
  projectDir: string,
  type: LearnablePattern['type'],
  pattern: string,
  pendingError?: string
): void {
 ...
 */
export function detectLearnablePattern(
  projectDir: string,
  toolName: string,
  toolInput: Record<string, unknown>,
  toolResponse: string
): LearnablePattern | null {
  const store = loadStore(projectDir);
  const now = new Date().toISOString();
