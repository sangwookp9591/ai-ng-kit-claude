 ...
 *
 * @type {Array<{ pattern: RegExp, level: 'allow'|'warn'|'block', message?: string }>}
 */
 ...
 * @returns {{ level: 'allow'|'warn'|'block', message?: string }}
 */
 ...
 * @param {object} [config] - aing config object
 * @param {object} [config.guardrail]
 * @param {string[]} [config.guardrail.freezeDirs] - Frozen directory prefixes
 * @returns {{ level: 'allow'|'block', message?: string }}
 */
export function checkFreeze(filePath, config) {
  const freezeDirs = config?.guardrail?.freezeDirs || [];
  if (freezeDirs.length === 0) return { level: 'allow' };
 ...
 * @param {{ level: string, message?: string }} result
 * @returns {string}
 */
export function formatCarefulResult(result) {
  if (result.level === 'allow') return '';
