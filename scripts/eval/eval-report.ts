...
import type { EvalResult, EvalRunSummary } from './eval-engine.js';
import type { RegressionAlert } from './llm-judge.js';
...
interface ReportSuccess {
  ok: true;
  data: { markdown: string; path: string };
}
...
function formatResultsTable(results: EvalResult[]): string {
...
function formatFindingsDetail(results: EvalResult[]): string {
  const failedResults = results.filter(r => !r.passed || r.findings.some(f => f.severity === 'error'));

  if (failedResults.length === 0) return '';

  const sections: string[] = ['## Findings Detail\n'];
...
function formatRegressionAlerts(results: EvalResult[]): string {
  const regressions: Array<{ skill: string; alert: RegressionAlert }> = [];
 ...
 */
export function formatEvalReport(summary: EvalRunSummary): string {
 ...
 */
export function saveEvalReport(summary: EvalRunSummary, projectDir?: string): ReportOutcome {
