...
function validateFrontmatterFields(fm: Frontmatter | null, findings: ValidationFinding[]): void {
  if (!fm) {
    findings.push({
...
function validateNoUnresolvedPlaceholders(content: string, findings: ValidationFinding[]): void {
...
function validateToolAllowlist(fm: Frontmatter | null, body: string, findings: ValidationFinding[]): void {
  if (!fm?.tools || !Array.isArray(fm.tools)) return;

  const declaredTools = new Set(fm.tools as string[]);
...
function validatePhaseCompleteness(body: string, findings: ValidationFinding[]): void {
...
function validateAgentReferences(body: string, projectDir: string, findings: ValidationFinding[]): void {
  ...
  while ((match = agentRefPattern.exec(body)) !== null) {
    ...
    if (['the', 'a', 'an', 'it', 'this', 'that', 'cc', 'claude', 'user'].includes(ref)) continue;
...
function validateContentQuality(content: string, findings: ValidationFinding[]): void {
  const lines = content.split('\n');
 ...
 */
export function runStaticValidation(skill: string, projectDir?: string): ValidationFinding[] {
 ...
 */
export function validateSkillContent(content: string, agentNames?: string[]): ValidationFinding[] {
  const findings: ValidationFinding[] = [];
  const { frontmatter, body } = parseFrontmatter(content);
