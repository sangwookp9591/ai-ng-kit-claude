...
/**
 * Cost mode levels — controls how aggressively models are downgraded.
 * @typedef {'quality'|'balanced'|'budget'} CostMode
 */
...
const RISK_ESCALATION_RULES = [
  ...
  { signal: (s) => s.fileCount > 20, minTier: 'opus', reason: '>20 files changed' },
  { signal: (s) => s.domainCount >= 3, minTier: 'sonnet', reason: '3+ domains touched' },
];
 ...
 * @param {string} agentName — agent role name (e.g., 'klay', 'milla')
 * @param {object} [signals] — complexity signals from task analysis
 * @param {object} [options]
 * @param {CostMode} [options.costMode='balanced'] — cost control mode
 * @param {string} [options.forceModel] — override all routing (user explicit)
 * @param {string} [options.context] — 'plan-review'|'verify'|'debug' etc.
 * @returns {{ model: string, reason: string, escalated: boolean }}
 */
export function routeModel(agentName, signals = {}, options = {}) {
  ...
  const hasRiskSignal = RISK_ESCALATION_RULES.some((rule) =>
    typeof rule.signal === 'function' ? rule.signal(safeSignals) : safeSignals[rule.signal]
  );
...
export function getCostMode() {
  ...
  if (env && ['quality', 'balanced', 'budget'].includes(env)) return env;
  return 'balanced';
}
