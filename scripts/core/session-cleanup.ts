 ...
 */
export function runSessionCleanup(projectDir: string, options?: CleanupOptions): CleanupResult {
  const result: CleanupResult = { cleaned: [], errors: [], skipped: [] };
  ...
  try {
    ...
    result.errors.push(`Failed to read state dir: ${(err as Error).message}`);
    return result;
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (file.endsWith('.tmp')) {
        if (dryRun) { result.skipped.push(`[dry-run] would remove: ${file}`); continue; }
        unlinkSync(filePath);
      ...
      if (file.endsWith('.lock')) {
        ...
        if (ageMs > lockStaleMs) {
          if (dryRun) { result.skipped.push(`[dry-run] would remove stale lock: ${file} (${Math.round(ageMs / 1000)}s)`); continue; }
          unlinkSync(filePath);
          result.cleaned.push(file);
        }
        continue;
      ...
      if (file.startsWith('handoff-') && file.endsWith('.md')) {
        ...
        if (ageMs > handoffMaxAgeMs) {
          if (dryRun) { result.skipped.push(`[dry-run] would archive: ${file}`); continue; }
...
function cleanByFileAge(filePath: string, fileName: string, maxAgeMs: number, dryRun: boolean, result: CleanupResult): void {
  try {
    const stat = statSync(filePath);
    const ageMs = Date.now() - stat.mtimeMs;
    if (ageMs > maxAgeMs) {
      if (dryRun) { result.skipped.push(`[dry-run] would remove: ${fileName} (${Math.round(ageMs / 86400000)}d old)`); return; }
      unlinkSync(filePath);
...
function deactivateStaleState(filePath: string, fileName: string, staleMs: number, dryRun: boolean, result: CleanupResult): void {
  try {
...
function deactivateStaleTeamSession(filePath: string, fileName: string, staleMs: number, dryRun: boolean, result: CleanupResult): void {
  try {
    const raw = readFileSync(filePath, 'utf-8');
    const data = JSON.parse(raw) as Record<string, unknown>;
    ...
    if (stage && terminalStages.includes(stage)) {
      if (dryRun) { result.skipped.push(`[dry-run] would clear terminal team-session (stage: ${stage})`); return; }
      writeState(filePath, {});
