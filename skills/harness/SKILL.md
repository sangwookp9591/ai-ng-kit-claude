---
name: harness
description: "하네스를 구성합니다. 도메인/프로젝트에 맞는 에이전트 팀을 설계하고, 전문 에이전트를 정의하며, 에이전트가 사용할 스킬을 생성하는 메타 스킬. (1) '하네스 구성해줘', '하네스 구축해줘', '하네스 설계해줘' 요청 시, (2) '에이전트 팀 설계', '스킬 생성', '오케스트레이션 구축' 요청 시, (3) 새로운 도메인/프로젝트에 대한 에이전트 기반 자동화 체계를 구축할 때, (4) 하네스 구성을 재구성하거나 확장할 때, (5) 'harness', 'agent team design', 'skill architect' 키워드가 포함된 요청 시 반드시 사용."
triggers: ["harness", "하네스", "에이전트 팀 설계", "스킬 생성", "오케스트레이션"]
---
...
```
/aing harness "코드 리뷰 팀 만들어"     ← 기본: 자동 설계 (find→설계→sim→check 통합)
/aing harness check                     ← 기존 하네스 검증
/aing harness find "리서치"             ← 패턴 검색/추천
/aing harness sim                       ← 데이터 흐름 시뮬레이션
/aing harness log                       ← 버전 이력 + A/B 비교
/aing harness chain "a → b → c"        ← 멀티 하네스 파이프라인
/aing harness fix                       ← 런타임 진단
```
...
| 인자 패턴 | 서브커맨드 | 동작 |
|-----------|----------|------|
| `check` | check | `validateHarness(cwd)` 실행 → 검증 결과 출력 |
| `find "keyword"` | find | `searchPatterns(keyword)` 실행 → 갤러리 결과 출력 |
| `sim` | sim | `simulateHarness(cwd)` 실행 → 시뮬레이션 결과 출력 |
| `log` | log | `getVersionList()` 실행 → 이력 출력 |
| `chain "a → b"` | chain | `composeHarnesses()` 실행 → 파이프라인 구성 출력 |
| `fix` | fix | `diagnoseHarness()` 실행 → 진단 결과 출력 |
| `"자연어 태스크"` | **(기본)** | Adaptive 자동 설계 → 6-Phase 워크플로우 실행 |

### 기본 동작 (Adaptive 자동 설계)

인자가 서브커맨드가 아닌 자연어 태스크이면, 다음을 **자동으로 순차 실행**한다:

1. **find**: 갤러리에서 유사 패턴 검색 → 추천 결과 표시
2. **analyzeTask**: complexity-scorer + 도메인 키워드로 최적 설계 자동 생성
3. **sim**: 생성된 설계의 데이터 흐름 시뮬레이션
4. **check**: 구조 검증
5. 사용자에게 설계 결과 표시 → 승인 시 6-Phase 워크플로우 실행

### TypeScript 모듈 경로

```
${CLAUDE_PLUGIN_ROOT}/scripts/harness/
├── harness-types.ts           ← 공유 타입
├── harness-parser.ts          ← 에이전트/스킬 MD 파서
├── adaptive-architect.ts      ← 자동 설계 엔진
├── validate-harness.ts        ← 구조 검증 엔진
├── harness-simulator.ts       ← 데이터 흐름 시뮬레이터
├── harness-gallery.ts         ← 패턴 DB + 추천
├── harness-evolution.ts       ← 버전 추적 + 메트릭
├── harness-composer.ts        ← 멀티 하네스 조합
└── harness-debugger.ts        ← 런타임 진단
```
...
**모든 에이전트는 반드시 `대상프로젝트/.claude/agents/{name}.md` 파일로 정의한다.** 에이전트 정의 파일 없이 Agent 도구의 prompt에 역할을 직접 넣는 것은 금지한다. 이유:
...
**모델 설정:** 에이전트는 `model: "opus"`를 권장한다. opus가 최고 품질을 보장하지만, 사용자의 구독 티어에 따라 opus를 사용할 수 없는 경우 `model: "sonnet"`으로 자동 대체한다. Agent 도구 호출 시 `model` 파라미터를 명시하되, API 에러(모델 미지원) 발생 시 sonnet으로 재시도한다.
...
각 에이전트를 `대상프로젝트/.claude/agents/{name}.md`에 정의한다. 필수 섹션: 핵심 역할, 작업 원칙, 입력/출력 프로토콜, 에러 핸들링, 협업. 에이전트 팀 모드에서는 `## 팀 통신 프로토콜` 섹션을 추가하여 메시지 수신/발신 대상과 작업 요청 범위를 명시한다.
...
각 에이전트가 사용할 스킬을 `대상프로젝트/.claude/skills/{name}/skill.md`에 생성한다. 상세 작성 가이드는 `references/skill-writing-guide.md` 참조.
...
2. **With-skill vs Without-skill 비교 실행** — 가능하면 스킬 있는 실행과 없는 실행을 병렬 수행하여 부가가치를 확인한다.
3. **결과 평가** — 산출물의 품질을 정성적(사용자 리뷰) + 정량적(assertion 기반) 으로 평가한다.
4. **반복 개선 루프** — 테스트 결과에서 문제가 발견되면 피드백을 **일반화**하여 스킬을 수정한다.
5. **반복 패턴 번들링** — 에이전트들이 공통으로 작성하는 코드가 발견되면 `scripts/`에 번들링한다.
...
- [ ] `대상프로젝트/.claude/agents/` — **에이전트 정의 파일 필수 생성** (빌트인 타입이라도 파일 생성 필수)
- [ ] `대상프로젝트/.claude/skills/` — 스킬 파일들 (skill.md + references/)
- [ ] 오케스트레이터 스킬 1개 (데이터 흐름 + 에러 핸들링 + 테스트 시나리오 포함)
- [ ] 실행 모드 명시 (에이전트 팀 또는 서브 에이전트)
- [ ] Agent 호출에 `model` 파라미터 명시 (opus 권장, 불가 시 sonnet)
...
- **QA 에이전트 가이드**: `references/qa-agent-guide.md` — 빌드 하네스에 QA 에이전트를 포함할 때 참조. 통합 정합성 검증 방법론, 경계면 버그 패턴, QA 에이전트 정의 템플릿 포함.
