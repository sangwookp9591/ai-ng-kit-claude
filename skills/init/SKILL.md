...
# /aing init — Project Initialization

GSD의 new-project 방식을 aing 스타일로 재해석한 프로젝트 초기화 스킬입니다.
질문 기반 문맥 수집 후 `.aing/project/` 에 프로젝트 문맥 파일을 생성합니다.
...
- `--reset` → 기존 `.aing/project/` 삭제 후 재초기화
...
USAGE:
  /aing init                   인터랙티브 프로젝트 초기화
  /aing init <project-name>    프로젝트명 지정 후 초기화
  /aing init --detect          코드베이스 자동 감지만 수행
  /aing init --reset           기존 초기화 데이터 삭제 후 재실행
  /aing init --help            이 도움말

OUTPUT:
  .aing/project/PROJECT.md      프로젝트 정의
  .aing/project/REQUIREMENTS.md 요구사항 체크리스트
  .aing/project/TECH-STACK.md   기술 스택 분석

NEXT STEPS:
  /aing plan <feature>   기능 계획 수립
  /aing do <task>        작업 실행
  /aing tdd start        TDD 사이클 시작
```

## Pre-Init Check: Already Initialized?

`.aing/project/PROJECT.md` 가 존재하는지 확인:

```bash
test -f .aing/project/PROJECT.md && echo "exists" || echo "fresh"
```
...
**Option 1 선택 시:** `.aing/project/PROJECT.md` 읽어서 출력 후 종료.
**Option 2 선택 시:** Phase 2 (Q1~Q3 스킵, Q4~Q5만) → Phase 3 (REQUIREMENTS.md만 재생성).
**Option 3 선택 시:** `rm -rf .aing/project/` 후 전체 Phase 실행.
**Option 4 선택 시:** 종료.
...
`templates/project.md` 템플릿을 참조하여 `.aing/project/PROJECT.md` 생성.
...
`templates/requirements.md` 템플릿을 참조하여 `.aing/project/REQUIREMENTS.md` 생성.
...
`templates/tech-stack.md` 템플릿을 참조하여 `.aing/project/TECH-STACK.md` 생성.
...
- 기존 `.aing/project/` 외부 파일은 절대 수정하지 않음
