...
MID/HIGH — Full Deliberation (5-7 에이전트):
  ...
  Phase 4: 오케스트레이터 직접 Synthesis (Able 재호출 없음)
    ↓
  Phase 5: Noa — Synthesis Verification
...
| Level | Score | Ryan | Able | Klay | Synthesis | Noa | Critic | Pre-mortem | Rollback | 합의 루프 | 에이전트 수 |
|-------|-------|------|------|------|-----------|-------|--------|------------|----------|----------|-----------|
...
**Fast Lane**: LOW(≤3)는 Noa/Critic 스킵. 3 에이전트 호출만으로 완료.
...
## Phase 5: Noa — Synthesis Verification (MID/HIGH only)

**LOW(≤3)**: 이 Phase를 스킵하고 Phase 7(Persist)로 직행한다.

**MID/HIGH**: Noa(sonnet)가 오케스트레이터의 synthesis 품질을 검증.

**중요**: Noa는 **플랜 텍스트**를 검증한다. 실제 코드 파일 변경을 확인하는 것이 아니다. 플랜 단계에서 코드가 아직 변경되지 않은 것은 정상이다. "파일이 변경되지 않았다"는 이유로 REVISE를 내리면 오판이다.

Noa(sonnet)가 synthesis 품질을 검증:
...
```
Noa REVISE → 오케스트레이터 직접 패치 (Able 재호출 금지) → Phase 5 (Noa 재검증)
Critic ITERATE → Targeted Patch (아래 참조)
```

| Level | 최대 반복 |
|-------|----------|
| low | 0회 (Noa/Critic 스킵) |
| mid | 2회 |
| high / deliberate | 2회 |
