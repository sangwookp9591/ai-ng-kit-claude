  ...
  try {
    switch (cmd) {
      ...
      case 'js': {
        ...
        if (!expr) return fail('Usage: js <expression>');
      ...
      case 'css': {
        if (args.length < 2) return fail('Usage: css <selector> <property>');
        ...
        if (value === null) return fail(`Element not found: ${selector}`);
        return ok(value);
      }

      case 'attrs': {
        if (args.length < 1) return fail('Usage: attrs <selector|@ref>');
        const target = args[0];
        let selector = target;
        ...
        if (attrs === null) return fail(`Element not found: ${selector}`);
        return ok(JSON.stringify(attrs, null, 2));
      }
      ...
      case 'console': {
        ...
        if (entries.length === 0) return ok('(no console messages)');
        const lines = entries.map(
          (e) => `[${e.level}] ${e.message}${e.source ? ` (${e.source})` : ''}`,
        );
      ...
      case 'network': {
        ...
        if (entries.length === 0) return ok('(no network requests)');
        const lines = entries.map(
          (e) => `${e.method} ${e.status} ${e.url}${e.duration ? ` (${e.duration}ms)` : ''}`,
        );
        return ok(lines.join('\n'));
      }
      ...
      case 'is': {
        if (args.length < 2) return fail('Usage: is <state> <selector>');
        const state = args[0];
      ...
      case 'dialog': {
        ...
        if (entries.length === 0) return ok('(no dialogs)');
        const lines = entries.map(
          (e) => `[${e.type}] ${e.message}${e.defaultValue ? ` (default: ${e.defaultValue})` : ''}`,
        );
        return ok(lines.join('\n'));
      }

      case 'eval': {
        if (args.length < 1) return fail('Usage: eval <file>');
