...
import type { CommandResult, SnapshotOptions } from './types.js';
...
export async function handleMetaCommand(
  ...
  try {
    switch (cmd) {
      ...
      case 'diff': {
        if (args.length < 2) return fail('Usage: diff <url1> <url2>');
      ...
      case 'tabs': {
        ...
        const lines = tabs.map(
          (t) => `${t.active ? '→' : ' '} [${t.id}] ${t.title || '(untitled)'} — ${t.url}`,
        );
        return ok(lines.join('\n') || '(no tabs)');
      }

      case 'tab': {
        if (args.length < 1) return fail('Usage: tab <id>');
        const id = parseInt(args[0], 10);
        if (isNaN(id)) return fail('Tab id must be a number');
      ...
      case 'chain': {
        ...
        if (!jsonStr) return fail('Usage: chain <json_array>');
        let commands: Array<{ cmd: string; args?: string[] }>;
        try {
        ...
        if (!Array.isArray(commands)) return fail('Expected JSON array');
        ...
        for (const c of commands) {
          ...
          if (!result.success) break; // Stop chain on error
        }
      ...
      case 'state': {
        if (args.length < 2) return fail('Usage: state save|load <name>');
      ...
      case 'inbox': {
        if (args.includes('--clear')) return ok('Inbox cleared.');
...
function parseSnapshotFlags(args: string[]): SnapshotOptions {
