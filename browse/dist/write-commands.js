...
export async function handleWriteCommand(cmd, args, bm) {
    const page = bm.getActivePage();
    try {
        switch (cmd) {
            case 'goto': {
                if (args.length < 1)
                    return fail('Usage: goto <url>');
            ...
            case 'click': {
                if (args.length < 1)
                    return fail('Usage: click <selector|@ref>');
            ...
            case 'fill': {
                if (args.length < 2)
                    return fail('Usage: fill <selector|@ref> <value>');
                const target = args[0];
            ...
            case 'select': {
                if (args.length < 2)
                    return fail('Usage: select <selector|@ref> <value>');
                const target = args[0];
                const value = args.slice(1).join(' ');
                if (target.startsWith('@')) {
                    const locator = await bm.resolveRef(target);
            ...
            case 'hover': {
                if (args.length < 1)
                    return fail('Usage: hover <selector|@ref>');
                const target = args.join(' ');
                if (target.startsWith('@')) {
                    const locator = await bm.resolveRef(target);
            ...
            case 'wait': {
                if (args.length < 1)
                    return fail('Usage: wait <selector|--networkidle|--load>');
                const target = args[0];
            ...
            case 'upload': {
                if (args.length < 2)
                    return fail('Usage: upload <selector> <file> [file2...]');
            ...
            case 'viewport': {
                if (args.length < 1)
                    return fail('Usage: viewport <WxH>');
                const match = args[0].match(/^(\d+)x(\d+)$/i);
                if (!match)
                    return fail('Format: WxH (e.g., 1920x1080)');
            ...
            case 'cookie': {
                if (args.length < 1)
                    return fail('Usage: cookie <name>=<value>');
                const eqIdx = args[0].indexOf('=');
                if (eqIdx === -1)
                    return fail('Format: name=value');
            ...
            case 'header': {
                if (args.length < 1)
                    return fail('Usage: header <name>:<value>');
            ...
            case 'useragent': {
                if (args.length < 1)
                    return fail('Usage: useragent <string>');
                const ua = args.join(' ');
            ...
            case 'cookie-import': {
                if (args.length < 1)
                    return fail('Usage: cookie-import <json-path>');
            ...
            case 'frame': {
                if (args.length < 1)
                    return fail('Usage: frame <sel|@ref|--name n|--url pattern|main>');
                const target = args[0];
                if (target === 'main') {
