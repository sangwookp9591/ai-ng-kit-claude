...
export class EvidenceCollector {
     ...
     */
    async capture(browse, label, metadata) {
     ...
     */
    async captureBeforeAfter(browse, label, action) {
        // Before
        ...
        try {
            ...
            return { ok: false, error: `Action failed: ${err.message}` };
        }
    ...
    static loadReport(projectRoot, sessionId) {
        ...
        try {
            ...
            return { ok: false, error: `Failed to load report: ${err.message}` };
        }
    }
}
// ---------------------------------------------------------------------------
...
// Formatting
// ---------------------------------------------------------------------------
function buildComparisonSummary(before, after, diff) {
    const lines = [];
    ...
    // Console error delta
    const newErrors = after.consoleErrors.filter((e) => !before.consoleErrors.includes(e));
