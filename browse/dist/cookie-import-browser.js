...
function findBrowser(name) {
    ...
    return BROWSER_PROFILES.find(p => p.name.toLowerCase().includes(lower) && existsSync(p.cookiePath)) ?? null;
}
/**
 ...
 */
export function importCookiesFromBrowser(browserName, domain) {
    const browser = findBrowser(browserName);
    if (!browser) {
        const available = listAvailableBrowsers();
        throw new Error(`Browser "${browserName}" not found. Available: ${available.join(', ') || 'none'}`);
    }
    ...
    try {
        ...
        throw new Error(`Cannot read cookie database for ${browser.name}. ` +
    ...
    try {
        ...
        query += ` LIMIT 1000;`;
        const result = execFileSync('sqlite3', ['-json', tmpDb, query], { stdio: 'pipe', maxBuffer: 10 * 1024 * 1024 }).toString().trim();
        if (!result || result === '[]') {
            return [];
        }
        ...
        // Clean up temp file
        try {
            execFileSync('rm', ['-f', tmpDb], { stdio: 'pipe' });
        }
        catch { /* ignore */ }
    }
}
//# sourceMappingURL=cookie-import-browser.js.map