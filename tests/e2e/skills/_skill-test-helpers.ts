...
export function scoreSkill(parsed: ParsedSkill): number {
  ...
  if (fm?.description) score += 10;
  // Triggers field (10pts)
  if (fm?.triggers && Array.isArray(fm.triggers) && fm.triggers.length > 0) score += 10;
  // Has sections (10pts)
  const headings = body.split('\n').filter(l => /^##\s/.test(l));
  if (headings.length >= 2) score += 10;
  // Has steps/phases or structured workflow (10pts)
  if (/^##\s+(Phase|Step)\s+\d+/im.test(body) || /^###\s+Step\s+\d+/im.test(body) || headings.length >= 3) score += 10;
  // No unresolved placeholders (10pts)
  if (!/\{\{[A-Z_]+\}\}/.test(content)) score += 10;
  // Content length adequate (10pts)
  const nonEmpty = content.split('\n').filter(l => l.trim().length > 0).length;
  if (nonEmpty >= 15) score += 10;
  // Agent or tool references exist (10pts)
  if (/agent|Agent|spawn|delegate|subagent|tool|Tool/i.test(body)) score += 10;
  // Error handling section (10pts)
  if (/error|failure|fallback|retry|abort|cancel|timeout|fail|exception/i.test(body)) score += 10;
...
export function assertFrontmatterValid(fm: Frontmatter | null): void {
  assert.ok(fm !== null, 'SKILL.md must start with YAML frontmatter (---)');
}

export function assertHasName(fm: Frontmatter, expectedName: string): void {
...
export function assertHasDescription(fm: Frontmatter, minLength: number = 5): void {
  ...
  assert.ok(
    desc.length >= minLength,
    `Description should be at least ${minLength} characters (got ${desc.length})`,
  );
}

export function assertHasTriggers(fm: Frontmatter): void {
...
export function assertNoPlaceholders(content: string): void {
  const placeholders = content.match(/\{\{[A-Z_]+\}\}/g);
  assert.strictEqual(
    placeholders,
    null,
    `Found unresolved placeholders: ${placeholders?.join(', ')}`,
  );
}

export function assertHasErrorHandling(body: string): void {
...
export function assertPhaseStructure(body: string): void {
  const headings = body.split('\n').filter(l => /^##\s/.test(l));
  assert.ok(headings.length >= 2, `Should have at least 2 sections (found ${headings.length})`);
}

export function assertPhasesHaveContent(body: string): void {
  ...
  for (const line of lines) {
    if (phasePattern.test(line)) {
      if (currentPhase !== null) {
        assert.ok(
          phaseContentLines >= 2,
          `"${currentPhase}" should have substantive content (found ${phaseContentLines} lines)`,
        );
      }
...
export function assertReferencesTools(body: string): void {
  ...
  assert.ok(
    toolRefs.length > 0 || /tool|Tool|mcp/i.test(body),
    'Should reference tools or describe tool usage',
  );
}

export function assertMinScore(parsed: ParsedSkill, threshold: number = 70): void {
