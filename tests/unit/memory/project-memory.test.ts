...
import {
  ...
  getMemorySummary,
} from '../../../scripts/memory/project-memory.js';
...
describe('saveMemory', () => {
  it('writes memory to state file', () => {
    ...
    expect((memoryStore[MEMORY_PATH] as Record<string, unknown>).techStack).toEqual({ db: 'postgres' });
  });
});
...
describe('addMemoryEntry', () => {
  it('adds entry to array section (patterns)', () => {
    ...
    const stored = memoryStore[MEMORY_PATH] as Record<string, unknown>;
    const patterns = stored.patterns as Array<{ content: string; confidence: number; source: string }>;
  ...
  it('adds entry with custom confidence and source', () => {
    ...
    const stored = memoryStore[MEMORY_PATH] as Record<string, unknown>;
    const pitfalls = stored.pitfalls as Array<{ content: string; confidence: number; source: string }>;
  ...
  it('appends to existing entries', () => {
    ...
    const stored = memoryStore[MEMORY_PATH] as Record<string, unknown>;
    const patterns = stored.patterns as Array<{ content: string }>;
  ...
  it('adds entry to object section (techStack)', () => {
    ...
    const stored = memoryStore[MEMORY_PATH] as Record<string, unknown>;
    const ts = stored.techStack as Record<string, string>;
  ...
  it('adds string to object section as note', () => {
    ...
    const stored = memoryStore[MEMORY_PATH] as Record<string, unknown>;
    const conv = stored.conventions as Record<string, string>;
  ...
  it('includes addedAt timestamp', () => {
    ...
    const stored = memoryStore[MEMORY_PATH] as Record<string, unknown>;
    const decisions = stored.decisions as Array<{ addedAt: string }>;
...
describe('applyConfidenceDecay', () => {
  it('does not decay user-stated entries', () => {
    ...
    const stored = memoryStore[MEMORY_PATH] as Record<string, unknown>;
    const patterns = stored.patterns as Array<{ confidence: number }>;
  ...
  it('removes entries with 0 confidence', () => {
    ...
    const stored = memoryStore[MEMORY_PATH] as Record<string, unknown>;
    const patterns = stored.patterns as Array<unknown>;
