...
import {
  ...
  threeWayComparison,
  formatThreeWay,
} from '../../../scripts/review/scope-drift.js';
...
describe('analyzeDrift', () => {
  ...
  it('detects out-of-scope files', () => {
    const result = analyzeDrift(
      { scope: ['src/auth'] },
      { files: ['src/auth/login.ts', 'src/billing/stripe.ts', 'src/email/send.ts'] },
    );
  ...
  it('detects missed goals', () => {
    const result = analyzeDrift(
      { goals: ['authentication', 'authorization', 'logging'], scope: ['src/'] },
      { files: ['src/auth.ts'] },
    );
  ...
  it('calculates correct drift percentage', () => {
    const result = analyzeDrift(
      { scope: ['src/core'] },
      { files: ['src/core/a.ts', 'tests/b.ts', 'docs/c.md', 'config/d.json'] },
    );
