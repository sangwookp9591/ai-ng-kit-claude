...
let costStore: Record<string, unknown> = {};
...
import {
  ...
  recordUsage,
  formatCostStatus,
} from '../../../scripts/guardrail/cost-ceiling.js';
...
describe('loadLimits', () => {
  ...
  it('merges user config with defaults', () => {
    vi.mocked(getConfig).mockImplementation((path: string, fallback: unknown) => {
      if (path === 'guardrail.costCeiling') return { maxTokensPerSession: 1000000 };
      return fallback;
    });
...
describe('initCostTracker', () => {
  it('creates initial tracker state', () => {
    ...
    const tracker = costStore[COST_PATH] as Record<string, unknown>;
