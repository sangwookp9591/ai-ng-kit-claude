...
let circuitStore: Record<string, unknown> = {};
...
vi.mock('../../../scripts/core/state.js', () => ({
  ...
  updateState: vi.fn((path: string, defaultVal: unknown, mutator: (data: unknown) => unknown) => {
    const current = circuitStore[path] ?? (typeof defaultVal === 'function' ? (defaultVal as () => unknown)() : defaultVal);
...
vi.mock('../../../scripts/core/config.js', () => ({
  ...
  getConfig: vi.fn((path: string, fallback: unknown) => {
    if (path === 'recovery.circuitBreakerThreshold') return 3;
    if (path === 'recovery.circuitBreakerResetMs') return 300000;
    return fallback;
  }),
...
import {
  ...
  isCircuitOpen,
  recordSuccess,
} from '../../../scripts/recovery/circuit-breaker.js';
...
describe('recordFailure', () => {
  ...
  it('truncates error message to 200 chars', () => {
    ...
    const stored = circuitStore[CIRCUIT_PATH] as Record<string, { lastError: string }>;
...
describe('isCircuitOpen', () => {
  ...
  it('transitions open to half-open after reset timeout', () => {
    ...
    const stored = circuitStore[CIRCUIT_PATH] as Record<string, { state: string }>;
...
describe('recordSuccess', () => {
  it('resets circuit to closed with zero failures', () => {
    ...
    const stored = circuitStore[CIRCUIT_PATH] as Record<string, { state: string; failures: number }>;
  ...
  it('is a no-op for unknown features', () => {
    ...
    const stored = circuitStore[CIRCUIT_PATH] as Record<string, unknown>;
...
describe('circuit breaker lifecycle', () => {
  it('completes full state transition cycle', () => {
    ...
    const stored = circuitStore[CIRCUIT_PATH] as Record<string, { openedAt: string }>;
    ...
    const final = circuitStore[CIRCUIT_PATH] as Record<string, { state: string; failures: number }>;
