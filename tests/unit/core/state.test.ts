...
let pathCounter = 0;
function uniquePath(prefix = '/tmp/state-test'): string {
 ...
 */
function setupLockSuccess(dataExistsFn?: (path: string) => boolean) {
  mockExistsSync.mockImplementation((p: unknown) => {
    const path = String(p);
    if (path.endsWith('.lock')) return false;
    if (dataExistsFn) return dataExistsFn(path);
    return false;
  });
}
...
describe('writeState', () => {
  ...
  it('returns ok:false when data write throws', () => {
    ...
    mockWriteFileSync.mockImplementation((...args: unknown[]) => {
      ...
      if (path.endsWith('.tmp')) throw new Error('ENOSPC');
      return undefined;
    });
...
describe('updateState', () => {
  ...
  it('uses default value when file does not exist', () => {
    ...
    const result = updateState(fp, { items: [] as string[] }, (data) => {
