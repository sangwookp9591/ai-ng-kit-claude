...
import {
  ...
  formatLandReport,
  type LandResult,
} from '../../../scripts/ship/land-orchestrator.js';
...
beforeEach(() => {
  ...
  mockExecSync.mockImplementation((cmd: string) => {
    ...
    if (cmdStr.includes('pr view') && cmdStr.includes('--json')) return APPROVED_PR;
    if (cmdStr.includes('pr checks')) return PASSING_CHECKS;
    if (cmdStr.includes('pr merge')) return '';
    if (cmdStr.includes('--jq .baseRefName')) return 'main';
    if (cmdStr.includes('git checkout')) return '';
    if (cmdStr.includes('git pull')) return '';
    if (cmdStr.includes('sleep')) return '';
    return '';
  });
});
...
describe('executeLandPipeline — pre-merge checks', () => {
  ...
  it('fails when gh CLI cannot fetch PR info', async () => {
    mockExecSync.mockImplementation((cmd: string) => {
      if (String(cmd).includes('pr view')) throw new Error('gh: not logged in');
      return '';
    });
...
describe('executeLandPipeline — merge strategy', () => {
  ...
  it('fails when merge command throws', async () => {
    mockExecSync.mockImplementation((cmd: string) => {
      ...
      if (cmdStr.includes('pr view') && cmdStr.includes('--json')) return APPROVED_PR;
      if (cmdStr.includes('pr checks')) return PASSING_CHECKS;
      if (cmdStr.includes('pr merge')) throw new Error('Merge failed: branch protection');
      return '';
    });
...
describe('executeLandPipeline — deploy wait', () => {
  it('skips when no URL available', async () => {
    vi.mocked(platformHealthCheck).mockReturnValue({ healthy: false, statusCode: 0 } as ReturnType<typeof platformHealthCheck>);
    const { detectPlatform } = await import('../../../scripts/ship/deploy-detect.js');
    vi.mocked(detectPlatform).mockReturnValue({ platform: 'unknown', config: null } as unknown as ReturnType<typeof detectPlatform>);
  ...
  it('passes when health check succeeds', async () => {
    vi.mocked(platformHealthCheck).mockReturnValue({ healthy: true, statusCode: 200 } as ReturnType<typeof platformHealthCheck>);
...
describe('executeLandPipeline — canary', () => {
  it('runs canary checks and records evidence', async () => {
    ...
    expect(addEvidence).toHaveBeenCalledWith(
      'canary-test',
      expect.objectContaining({ type: 'deploy', source: 'land-orchestrator' }),
      '/tmp/land-test-13',
    );
  });
  ...
  it('reports canary failure', async () => {
    ...
    } as unknown as ReturnType<typeof runCanaryLoop>);
