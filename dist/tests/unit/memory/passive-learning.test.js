...
describe('capturePassive — basic', () => {
    it('호출 시 pitfalls에 항목이 추가된다', () => {
        ...
        expect(mockAddMemoryEntry).toHaveBeenCalledWith('pitfalls', '세션 종료 감지', '/project', { confidence: 7, source: 'passive' });
    });
    it('context가 있으면 [context] prefix가 붙는다', () => {
        capturePassive({ trigger: 'guardrail-denial', content: 'rm -rf 차단', context: 'bash' }, '/project');
        expect(mockAddMemoryEntry).toHaveBeenCalledWith('pitfalls', '[bash] rm -rf 차단', '/project', { confidence: 7, source: 'passive' });
    });
...
describe('capturePassive — dedup', () => {
    ...
    it('다른 content라면 추가된다', () => {
        ...
        capturePassive({ trigger: 'guardrail-denial', content: '다른 내용', context: 'bash' }, '/project');
...
//# sourceMappingURL=passive-learning.test.js.map