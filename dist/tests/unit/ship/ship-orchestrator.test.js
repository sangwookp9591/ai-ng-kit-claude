...
vi.mock('../../../scripts/ship/version-bump.js', () => ({
    determineBumpType: vi.fn(({ hasBreaking, hasNewFeature }) => {
        if (hasBreaking)
            return 'major';
        if (hasNewFeature)
            return 'minor';
...
beforeEach(() => {
    ...
    vi.mocked(buildDashboard).mockReturnValue({ verdict: 'CLEARED', verdictReason: 'All checks passed' });
    vi.mocked(isGhAvailable).mockReturnValue(true);
    vi.mocked(initShip).mockImplementation(() => undefined);
    ...
    mockExecSync.mockImplementation((cmd) => {
        ...
        if (cmdStr.includes('status --porcelain'))
            return '';
        if (cmdStr.includes('git fetch'))
            return '';
        if (cmdStr.includes('git merge'))
            return '';
        if (cmdStr.includes('git log'))
            return 'abc123 feat: add login\ndef456 fix: typo';
        ...
        if (cmdStr.includes('npm test') || cmdStr.includes('node --test'))
            return 'Tests: 10 passed, 0 failed';
        return '';
    });
});
// ── Preflight ────────────────────────────────────────────────────────────
    ...
    it('fails when on base branch', async () => {
        mockExecSync.mockImplementation((cmd) => {
            if (String(cmd).includes('branch --show-current'))
                return 'main';
            if (String(cmd).includes('rev-parse --verify origin/main'))
                return 'abc';
...
describe('executeShipPipeline — merge base', () => {
    ...
    it('fails on merge conflict', async () => {
        mockExecSync.mockImplementation((cmd) => {
            ...
            if (cmdStr.includes('rev-parse --verify origin/main'))
                return 'abc';
            if (cmdStr.includes('git merge origin/'))
                throw new Error('CONFLICT');
            if (cmdStr.includes('status --porcelain') && mockExecSync.mock.calls.length > 3)
                return 'UU src/file.ts';
            if (cmdStr.includes('status --porcelain'))
                return '';
            if (cmdStr.includes('git fetch'))
                return '';
            if (cmdStr.includes('merge --abort'))
                return '';
            return '';
        });
        const result = await executeShipPipeline({
...
describe('executeShipPipeline — version bump', () => {
    it('detects breaking change for major bump', async () => {
        mockExecSync.mockImplementation((cmd) => {
            const cmdStr = String(cmd);
            if (cmdStr.includes('branch --show-current'))
                return 'feature/break';
            if (cmdStr.includes('rev-parse'))
                return 'abc';
            if (cmdStr.includes('status --porcelain'))
                return '';
            if (cmdStr.includes('git fetch') || cmdStr.includes('git merge'))
                return '';
            if (cmdStr.includes('git log'))
                return 'abc BREAKING: remove API\ndef fix: typo';
            if (cmdStr.includes('git diff'))
                return '';
            if (cmdStr.includes('git add') || cmdStr.includes('git commit') || cmdStr.includes('git push'))
                return '';
        ...
        expect(determineBumpType).toHaveBeenCalledWith(expect.objectContaining({ hasBreaking: true }));
    });
    it('detects feat commit for minor bump', async () => {
        ...
        expect(determineBumpType).toHaveBeenCalledWith(expect.objectContaining({ hasNewFeature: true }));
    });
});
// ── Pre-landing review ───────────────────────────────────────────────────
...
//# sourceMappingURL=ship-orchestrator.test.js.map