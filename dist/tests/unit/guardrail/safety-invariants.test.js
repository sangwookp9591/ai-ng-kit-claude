...
vi.mock('../../../scripts/core/state.js', () => ({
    ...
    updateState: vi.fn((path, defaultVal, mutator) => {
        const current = invariantStore[path] ?? (typeof defaultVal === 'function' ? defaultVal() : JSON.parse(JSON.stringify(defaultVal)));
...
import { loadInvariants, checkStepLimit, checkFileChangeLimit, checkForbiddenPath, checkErrorLimit, resetErrorCount, resetTrackers, getTrackerStatus, } from '../../../scripts/guardrail/safety-invariants.js';
...
describe('loadInvariants', () => {
    ...
    it('merges custom forbidden paths with defaults', () => {
        vi.mocked(getConfig).mockImplementation((path, fallback) => {
            if (path === 'guardrail.invariants')
                return { forbiddenPaths: ['/custom/path/'] };
            return fallback;
        });
        const inv = loadInvariants();
    ...
    it('handles boolean config', () => {
        vi.mocked(getConfig).mockImplementation((path, fallback) => {
            if (path === 'guardrail.invariants')
                return { requireTestBeforeCommit: true };
            return fallback;
        });
        const inv = loadInvariants();
...
//# sourceMappingURL=safety-invariants.test.js.map