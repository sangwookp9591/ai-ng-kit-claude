...
import { resolveProfile, isAgentAllowed, filterWorkers, checkTokenLimit, AGENT_CATEGORIES, } from '../../../scripts/routing/profile-resolver.js';
...
function makeWorker(name, agent = 'executor', model = 'sonnet', role = 'dev') {
...
describe('AGENT_CATEGORIES', () => {
    it('exports a non-empty map with expected categories', () => {
        ...
        expect(Object.keys(AGENT_CATEGORIES)).toEqual(expect.arrayContaining(['leadership', 'backend', 'frontend', 'design', 'aiml', 'special']));
    });
...
describe('isAgentAllowed — category filtering', () => {
    it('blocks agent whose only category is disabled', () => {
        const agents = makeAgents({ categories: { aiml: false } });
    ...
    it('allows agent when at least one of its categories is enabled (OR semantics)', () => {
        ...
        const agents = makeAgents({
            categories: { frontend: false, backend: true, design: false },
        });
        expect(isAgentAllowed('derek', agents)).toBe(true);
    });
    it('blocks derek when all its categories are disabled', () => {
        const agents = makeAgents({
            categories: { frontend: false, backend: false, design: false },
        });
...
describe('filterWorkers — basic filtering', () => {
    ...
    it('sam is always included regardless of allowedAgents (protected)', () => {
        const profile = {
            allowedAgents: new Set(), // sam not in allowed set
            costMode: 'balanced',
            maxTeamSize: 10,
            tokenLimit: null,
        };
...
describe('AGENT_CATEGORIES ↔ TEAM_PRESETS consistency', () => {
    ...
    const allKnownAgents = new Set(Object.values(AGENT_CATEGORIES).flatMap((entries) => entries.map((e) => e.name)));
    // Flatten all worker names registered in TEAM_PRESETS
    const allPresetWorkerNames = Object.values(TEAM_PRESETS).flatMap((preset) => preset.workers.map((w) => w.name));
...
//# sourceMappingURL=profile-resolver.test.js.map