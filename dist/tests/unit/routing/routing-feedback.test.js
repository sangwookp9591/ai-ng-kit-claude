...
vi.mock('../../../scripts/core/config.js', () => ({
    getConfig: vi.fn((_key, defaultVal) => defaultVal),
}));
import { recordRouting, getSuccessRate, adjustConfidence, } from '../../../scripts/routing/routing-history.js';
// ─── Helpers ─────────────────────────────────────────────────────────────────
...
describe('recordRouting — atomic updateState', () => {
    ...
    it('mutator가 entry를 배열에 추가하고 retention 제한을 적용한다', () => {
        mockUpdateState.mockImplementation((_path, _default, mutator) => {
            const existing = makeHistory(Array.from({ length: 50 }, () => ({ outcome: 'success' })));
        ...
        const input = makeHistory(Array.from({ length: 50 }, () => ({ outcome: 'success' })));
...
describe('adjustConfidence — 공식 및 범위', () => {
    ...
    function setRouteHistory(route, entries) {
        mockReadState.mockReturnValue({
            ok: true,
    ...
    it('rate=0%: base * 0.875 (하한)', () => {
        setRouteHistory('debug', Array.from({ length: 5 }, () => ({ outcome: 'fail' })));
        const base = 0.90;
    ...
    it('rate=100%: base * 1.125 (상한)', () => {
        setRouteHistory('debug', Array.from({ length: 6 }, () => ({ outcome: 'success' })));
        const base = 0.90;
        const result = adjustConfidence(base, 'debug');
    ...
    it('조정값이 [0.875*base, 1.125*base] 범위 내에 있음', () => {
        ...
        rates.forEach(rate => {
            ...
            const entries = [
                ...Array.from({ length: successCount }, () => ({ outcome: 'success' })),
                ...Array.from({ length: 10 - successCount }, () => ({ outcome: 'fail' })),
            ];
...
//# sourceMappingURL=routing-feedback.test.js.map