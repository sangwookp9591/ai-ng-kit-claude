...
vi.mock('../../../scripts/core/state.js', () => ({
    readState: vi.fn((path) => {
        if (pdcaStore[path])
            return { ok: true, data: pdcaStore[path] };
...
vi.mock('../../../scripts/core/config.js', () => ({
    ...
    getConfig: vi.fn((path, fallback) => {
        if (path === 'pdca.matchRateThreshold')
            return 90;
        if (path === 'pdca.maxIterations')
            return 5;
        return fallback;
    }),
...
import { startPdca, advancePdca, getPdcaStatus, completePdca, resetPdca, getScalingProfile, STAGES, STAGE_DESCRIPTIONS, } from '../../../scripts/pdca/pdca-engine.js';
...
describe('startPdca', () => {
    it('creates new feature at plan stage', () => {
        ...
        const state = pdcaStore[STATE_PATH];
    ...
    it('handles backward compat (string as projectDir)', () => {
        const result = startPdca('bc-test', '/tmp/pdca-test');
        expect(result.ok).toBe(true);
    });
...
describe('getPdcaStatus', () => {
    ...
    it('returns feature status when specified', () => {
        ...
        const status = getPdcaStatus('status-test', '/tmp/pdca-test');
    ...
    it('returns full state when no feature specified', () => {
        ...
        const state = getPdcaStatus(undefined, '/tmp/pdca-test');
...
//# sourceMappingURL=pdca-engine.test.js.map