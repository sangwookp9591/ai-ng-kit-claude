...
import { checkFeedbackViolation, checkAutonomyRisk, recordFeedback, runRealityCheck, writeRealityCheckFlag, clearRealityCheckFlag, readRealityCheckFlag, } from '../../../scripts/hooks/reality-check.js';
...
function feedbackPath(dir) {
    return join(dir, '.aing', 'logs', 'feedback-memory.jsonl');
}
function writeFeedbackEntry(dir, keyword, isFalsePositive = false) {
    const entry = {
...
function writeTaskFile(dir, files) {
    const task = {
        id: 'task-test',
...
describe('runRealityCheck — denial-tracker integration', () => {
    ...
    it('calls recordDenial when feedback violation detected', () => {
        ...
        expect(mockRecordDenial).toHaveBeenCalledWith(expect.objectContaining({ ruleId: 'rc-feedback-violation', action: 'warn' }), dir);
    });
    it('calls recordDenial with block when destructive risk detected', () => {
        ...
        expect(mockRecordDenial).toHaveBeenCalledWith(expect.objectContaining({ ruleId: 'rc-destructive-unconfirmed', action: 'block' }), dir);
    });
...
//# sourceMappingURL=reality-check.test.js.map