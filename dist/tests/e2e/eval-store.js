 ...
 */
export function saveEval(result) {
    ensureDir();
 ...
 */
export function loadLatestEval(testName) {
    ensureDir();
 ...
 */
export function listEvals(limit = 50) {
    ensureDir();
    const files = readdirSync(EVALS_DIR)
 ...
 */
export function compareEvals(baseline, current) {
    const baseMap = new Map();
    for (const b of baseline)
        baseMap.set(b.testName, b);
    const improved = [];
    const regressed = [];
    const unchanged = [];
    const newTests = [];
...
export function formatComparison(cmp) {
    ...
    if (cmp.unchanged.length > 0) {
        ...
        for (const t of cmp.unchanged)
            lines.push(`  = ${t}`);
    }
    if (cmp.newTests.length > 0) {
        lines.push(`\n[New] (${cmp.newTests.length})`);
        for (const t of cmp.newTests)
            lines.push(`  * ${t}`);
    }
    const total = cmp.improved.length + cmp.regressed.length + cmp.unchanged.length + cmp.newTests.length;
