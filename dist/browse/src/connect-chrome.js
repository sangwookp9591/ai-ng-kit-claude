...
export function findChrome() {
    ...
    for (const app of apps) {
        if (existsSync(app.path))
            return { execPath: app.path, name: app.name };
    }
    ...
    for (const app of apps) {
        ...
        if (existsSync(userPath))
            return { execPath: userPath, name: app.name };
    }
    return null;
}
/**
 ...
 */
export function launchChromeWithDebug(port = 9223, url) {
    ...
    for (let i = 0; i < 20; i++) {
        try {
            const result = execSync(`curl -s http://127.0.0.1:${port}/json/version`, { stdio: 'pipe', timeout: 2000 }).toString();
...
export function focusChrome() {
    if (platform() !== 'darwin')
        return false;
    try {
        execSync(`osascript -e 'tell application "Google Chrome" to activate'`, { stdio: 'pipe', timeout: 3000 });
        return true;
    }
    catch {
        return false;
...
//# sourceMappingURL=connect-chrome.js.map