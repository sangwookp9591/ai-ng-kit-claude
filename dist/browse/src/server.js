...
export class BrowseServer {
    ...
    start() {
        return new Promise((resolve, reject) => {
            ...
            server.listen(this.config.port, '127.0.0.1', () => {
                this.server = server;
                console.log(`[browse] Server listening on http://127.0.0.1:${this.config.port}`);
    ...
    handleActivity(req, res, startTime) {
        // Parse limit from query string
        const urlObj = new URL(req.url ?? '/', `http://127.0.0.1`);
        const limitStr = urlObj.searchParams.get('limit');
        const limit = limitStr ? parseInt(limitStr, 10) : 50;
        const clampedLimit = Math.min(Math.max(1, limit), 100);
        const events = this.activity.getRecentActivity(clampedLimit);
    ...
    handleSessions(req, res, startTime) {
    ...
    handleCommands(req, res, startTime) {
    ...
    handleCommand(req, res, startTime) {
        let body = '';
        let bodySize = 0;
        req.on('data', (chunk) => {
        ...
        req.on('end', async () => {
            try {
                const { cmd, args: cmdArgs, sessionId: clientSessionId } = JSON.parse(body);
                if (!cmd) {
                ...
                const sessionId = this.resolveSession(clientSessionId ?? req.headers['x-session-id'], req);
                ...
                if (!this.bm.isLaunched() && !this.isRestarting) {
                    ...
                    if (!restarted) {
                        this.sendJson(res, 503, {
                            success: false,
                            output: '',
                            error: 'Browser is disconnected and could not be restarted',
                        });
                ...
                const result = await dispatchCommand(cmd, cmdArgs ?? [], this.bm, () => this.shutdown());
                const cmdDuration = Date.now() - cmdStart;
                // Track
                this.commandsProcessed++;
                this.touchSession(sessionId);
                // Record activity
                this.activity.recordCommand(cmd, cmdArgs ?? [], result.success, cmdDuration, sessionId, result.error);
                // Respond
    ...
    // Session Management
    // -----------------------------------------------------------------------
    resolveSession(clientSessionId, req) {
    ...
    async restartBrowser() {
        ...
        for (let attempt = 1; attempt <= BROWSER_RESTART_MAX_ATTEMPTS; attempt++) {
            console.log(`[browse] Browser disconnected. Restart attempt ${attempt}/${BROWSER_RESTART_MAX_ATTEMPTS}...`);
            ...
            try {
                ...
                if (attempt < BROWSER_RESTART_MAX_ATTEMPTS) {
                    await new Promise((r) => setTimeout(r, BROWSER_RESTART_DELAY_MS * attempt));
                }
            }
        }
        this.isRestarting = false;
        this.activity.record('error', 'Browser restart failed after all attempts');
        return false;
    }
    // -----------------------------------------------------------------------
    ...
    getHealthStatus() {
        if (!this.bm.isLaunched())
            return 'error';
        if (this.browserRestarts > 0)
            return 'degraded';
        return 'ok';
    }
    logRequest(req, status, startTime, sessionId) {
        ...
        console.log(`[browse] ${method} ${url} ${status} ${durationMs}ms${sessionTag}`);
    }
    // -----------------------------------------------------------------------
 ...
 */
export function startServer(port, token, bm, options) {
