...
async function checkExistingServer() {
    ...
    if (!isPidAlive(state.pid)) {
        console.log(`[browse] Stale state detected (PID ${state.pid} not alive). Cleaning up.`);
        clearState();
        return null;
    }
    ...
    console.log(`[browse] Server PID ${state.pid} alive but not responding. Cleaning up.`);
    clearState();
    return null;
}
// ---------------------------------------------------------------------------
...
async function main() {
    ...
    if (existing) {
        console.log(`[browse] Server already running on port ${existing.port} (PID ${existing.pid})`);
