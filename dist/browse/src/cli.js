...
function sendCommand(state, cmd, args) {
...
async function ensureServer(headless = true) {
    ...
    const child = spawn(process.execPath, [serverScript, String(port), token, headless ? 'headless' : 'headed'], {
        detached: true,
        stdio: 'ignore',
        env: { ...process.env },
    });
