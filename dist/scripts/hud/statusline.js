...
const AGENTS = {
    ...
    derek: { label: 'Derek', role: 'Motion', color: BRIGHT_YELLOW },
    rowan: { label: 'Rowan', role: 'Mobile', color: WHITE },
    wizard: { label: 'Iron', role: 'Web Frontend', color: BLUE },
...
function parseActiveAgents(transcriptPath) {
    ...
    try {
        ...
        const agentSpawns = new Map(); // tool_use_id -> agent name
...
function getUpdateInfo(cwd) {
    ...
    `;
        const child = execFile('node', ['-e', script], { detached: true }, () => { });
        child.unref();
 ...
 */
export function getTeamStatusLine(cwd) {
    try {
