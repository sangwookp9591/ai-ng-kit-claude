...
function collectFiles(searchPath, lang, maxFiles = 1000) {
...
// Helpers
// ---------------------------------------------------------------------------
function surroundingLines(content, startLine, endLine, ctx = 3) {
 ...
 */
export async function astGrepSearch(pattern, options = {}) {
 ...
 */
export async function astGrepReplace(pattern, replacement, options = {}) {
    const sg = await getSg();
    if (!sg) {
        throw new Error(`@ast-grep/napi is not available: ${sgLoadError}`);
    }
