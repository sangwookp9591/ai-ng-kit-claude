...
export function gatherMetrics(window = '7d', projectDir) {
    ...
    try {
        // Commits with stats
        const logRaw = execSync(`git log --since="${since}" --pretty=format:"%H|%s|%an|%ai" --shortstat`, { cwd: dir, encoding: 'utf-8', timeout: 10000 }).trim();
        // Parse commits
        const commits = [];
        ...
        let shortlog = '';
        try {
            shortlog = execSync(`git shortlog -sn --since="${since}" --no-merges`, { cwd: dir, encoding: 'utf-8', timeout: 10000 }).trim();
        }
        catch { }
        ...
        let hotspots = [];
        try {
            const hotspotRaw = execSync(`git log --since="${since}" --pretty=format: --name-only | sort | uniq -c | sort -rn | head -15`, { cwd: dir, encoding: 'utf-8', timeout: 10000 }).trim();
...
//# sourceMappingURL=retro-engine.js.map