 ...
 */
import { getCostMode } from './model-router.js';
...
function computeCostPenalty(route, costMode) {
    if (costMode === 'quality')
        return 0;
 ...
 */
export function computeDenseScore(route, options = {}) {
    ...
    // Range: roughly [0, 1]
    const denseScore = Math.max(0, Math.min(1, 0.5 + (historySuccessRate - 0.5) * 0.4 * historyWeight - costPenalty));
    return {
        denseScore,
 ...
 */
export function fuseScores(route, sparseScore, denseScore) {
 ...
 */
export function rerank(candidates, options = {}) {
    return candidates
        .map(c => {
