 ...
 */
import { startSession, diagnose, recordAnswer, generateSummary, } from './knowledge-tracker.js';
// ── Concept Extraction ──
/**
 * 작업 설명에서 관련 프로그래밍 개념을 추출한다.
 * 키워드 매칭 기반 — LLM 호출 없이 빠르게 동작.
 */
export function extractConcepts(taskDescription) {
    const conceptMap = {
        // TypeScript
 ...
 */
export function generateQuestionTemplate(concept, category, difficulty, _codeContext) {
    ...
    // 난이도에 맞는 질문 유형 선택
    const types = ['why', 'what-if', 'compare', 'predict', 'debug', 'refactor'];
...
function generateHints(concept, difficulty) {
 ...
 */
export function createTeachingPlan(projectDir, task) {
 ...
 * Teacher 세션을 시작한다.
 */
export function startTeachingSession(projectDir, task) {
 ...
 */
export function processAnswer(projectDir, session, questionId, outcome, userAnswer) {
    // 현재 질문 정보 찾기
 ...
 * 세션 종료 시 학습 요약을 생성한다.
 */
export function endTeachingSession(projectDir, session) {
...
// ── Helpers ──
function lowerDifficulty(level) {
    if (level === 'advanced')
        return 'intermediate';
    return 'beginner';
}
function raiseDifficulty(level) {
    if (level === 'beginner')
        return 'intermediate';
    return 'advanced';
}
//# sourceMappingURL=teacher-engine.js.map