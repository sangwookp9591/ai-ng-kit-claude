...
// ── File I/O (atomic) ──
function getKnowledgePath(projectDir) {
...
export function loadKnowledge(projectDir) {
...
export function saveKnowledge(projectDir, profile) {
    const path = getKnowledgePath(projectDir);
    ensureDir(path);
 ...
 * 개념에 대한 질문 결과를 기록한다.
 */
export function recordAnswer(projectDir, result) {
 ...
 */
export function startSession(projectDir) {
    const profile = loadKnowledge(projectDir);
    const now = new Date();
 ...
 */
export function diagnose(projectDir, relatedConcepts) {
    const profile = loadKnowledge(projectDir);
    const knownConcepts = [];
 ...
 * 학습 요약 리포트를 생성한다.
 */
export function generateSummary(projectDir) {
    const profile = loadKnowledge(projectDir);
...
// ── Helpers ──
function calculateDifficulty(profile) {
    if (profile.concepts.length === 0)
        return 'intermediate';
...
function applyConfidenceDecay(profile) {
...
function updateAreas(profile) {
    ...
    for (const [category, confidences] of byCategory) {
        ...
        if (avg >= MASTERY_THRESHOLD)
            profile.strongAreas.push(category);
        else if (avg < WEAK_THRESHOLD)
            profile.weakAreas.push(category);
    }
}
