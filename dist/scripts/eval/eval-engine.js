...
export var EvalTier;
(function (EvalTier) {
    EvalTier["STATIC"] = "STATIC";
    EvalTier["E2E"] = "E2E";
    EvalTier["LLM_JUDGE"] = "LLM_JUDGE";
})(EvalTier || (EvalTier = {}));
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function runStaticTier(skill, projectDir) {
...
// ---------------------------------------------------------------------------
function runE2ETier(skill, projectDir) {
    const start = Date.now();
    const findings = [];
    let score = 0;
    const maxScore = 100;
    try {
        ...
        const result = execFileSync('claude', ['-p', testPrompt, '--max-turns', '1'], {
            cwd: projectDir,
            timeout: 120_000,
            encoding: 'utf-8',
...
// Tier 3: LLM-as-judge
// ---------------------------------------------------------------------------
function runLlmJudgeTier(skill, projectDir) {
    const start = Date.now();
 ...
 */
export function runEval(skill, tier, projectDir) {
 ...
 */
export function runEvalSuite(tiers, projectDir, skillFilter) {
    const dir = projectDir || process.cwd();
...
//# sourceMappingURL=eval-engine.js.map