 ...
 */
export function checkGoalAchievement(_projectDir, goalDescription, assertions) {
...
export function deriveAssertions(goalDescription) {
    ...
    if (/로그인|login|auth|인증/.test(goal)) {
        assertions.push({ claim: '유효한 자격증명으로 로그인 가능', verified: false, evidence: '' }, { claim: '로그인 후 세션/토큰 발급', verified: false, evidence: '' }, { claim: '잘못된 자격증명 시 에러 반환', verified: false, evidence: '' });
    }
    // CRUD / API 관련
    if (/추가|생성|create|add|등록/.test(goal)) {
        assertions.push({ claim: '새 항목 생성 API 존재', verified: false, evidence: '' }, { claim: '생성된 항목 저장소에 반영됨', verified: false, evidence: '' });
    }
    if (/수정|업데이트|update|edit/.test(goal)) {
        assertions.push({ claim: '항목 수정 API 존재', verified: false, evidence: '' }, { claim: '수정 결과 저장소에 반영됨', verified: false, evidence: '' });
    }
    if (/삭제|제거|delete|remove/.test(goal)) {
        assertions.push({ claim: '항목 삭제 API 존재', verified: false, evidence: '' }, { claim: '삭제 후 항목 조회 불가', verified: false, evidence: '' });
    }
    if (/조회|검색|search|list|목록/.test(goal)) {
        assertions.push({ claim: '목록 조회 API 존재', verified: false, evidence: '' }, { claim: '검색 조건 필터링 동작', verified: false, evidence: '' });
    }
    // 테스트 관련
    if (/테스트|test|tdd/.test(goal)) {
        assertions.push({ claim: '테스트 파일 존재', verified: false, evidence: '' }, { claim: '모든 테스트 통과', verified: false, evidence: '' });
    }
    // assertion이 없는 경우 범용 assertion
    if (assertions.length === 0) {
        assertions.push({ claim: `"${goal}" 기능이 구현됨`, verified: false, evidence: '' }, { claim: `관련 테스트가 통과함`, verified: false, evidence: '' });
    }
    return assertions;
}
/**
...
//# sourceMappingURL=goal-checker.js.map