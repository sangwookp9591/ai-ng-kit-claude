 ...
 * Note: Uses execSync for git commands with fixed strings (no user input), safe from injection.
...
function getBaseBranch(dir) {
    try {
        execSync('git rev-parse --verify origin/main', { cwd: dir, encoding: 'utf-8' });
        return 'main';
    }
    catch { }
    try {
        execSync('git rev-parse --verify origin/master', { cwd: dir, encoding: 'utf-8' });
        return 'master';
    }
    catch { }
    return 'main';
...
function getStatus(dir) {
    try {
        return execSync('git status --porcelain', { cwd: dir, encoding: 'utf-8' }).trim();
    }
    catch {
        return '';
    }
