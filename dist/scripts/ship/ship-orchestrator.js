...
export async function executeShipPipeline(options) {
    const { feature, baseBranch = detectBaseBranch(options.projectDir), dryRun = false, skipTests = false, projectDir, } = options;
    ...
    try {
        ...
        if (!preflightResult.ok)
            return { success: false, state: getShipState(dir), error: preflightResult.reason };
        ...
        if (!mergeResult.ok)
            return { success: false, state: getShipState(dir), error: mergeResult.reason };
        // Step 3: Run tests
        if (skipTests) {
            ...
            if (!testResult.ok)
                return { success: false, state: getShipState(dir), error: testResult.reason };
        }
        ...
        if (!reviewResult.ok && reviewResult.critical)
            return { success: false, state: getShipState(dir), error: reviewResult.reason };
        // Step 5: Version bump
...
function runMergeBase(dir, baseBranch, dryRun) {
    if (dryRun)
        return { ok: true, action: 'dry-run skip' };
    try {
...
function runChangelog(dir, newVersion, _baseBranch, dryRun) {
...
function runPushAndPR(dir, branch, baseBranch, feature, versionResult, changelogResult) {
    try {
        ...
        const prOutput = execSync(`gh pr create --title "${title.replace(/"/g, '\\"')}" --base ${baseBranch} --body "${body.replace(/"/g, '\\"').replace(/\n/g, '\\n')}"`, { cwd: dir, encoding: 'utf-8', timeout: 30000 }).trim();
...
//# sourceMappingURL=ship-orchestrator.js.map