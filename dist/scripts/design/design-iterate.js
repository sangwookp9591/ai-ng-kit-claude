 ...
 */
export function iterateDesign(system, feedback, iteration = 1) {
    log.info(`Design iteration #${iteration} with ${feedback.length} feedback items`);
    const changes = [];
    const tokens = JSON.parse(JSON.stringify(system.tokens));
 ...
 */
export function autoFixDesign(system) {
    const feedback = [];
 ...
 */
export function formatIteration(result) {
    const lines = [];
...
//# sourceMappingURL=design-iterate.js.map