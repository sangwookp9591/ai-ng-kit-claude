 ...
 */
export function logTokenUsage(entry, projectDir) {
    ...
    try {
        ...
        log.warn(`Token usage write failed: ${err.message}`);
    }
}
/**
 ...
 */
export function getTokenSummary(projectDir) {
    const dir = projectDir || process.cwd();
    ...
    for (const entry of entries) {
        ...
        // byStage
        if (!byStage[stage])
            byStage[stage] = { tokens: 0, agents: 0, duration: 0, agentTokens: {} };
        ...
        // byAgent
        if (!byAgent[agent])
            byAgent[agent] = { tokens: 0, tasks: 0, duration: 0 };
 ...
 *     total:  ~96.8k tokens
 */
export function formatTokenReport(summary) {
 ...
 */
export function checkSessionTokenLimit(limit, projectDir) {
    if (limit === null) {
 ...
 */
export function clearTokenUsage(projectDir) {
    const dir = projectDir || process.cwd();
    const filePath = join(dir, TELEMETRY_DIR, TOKEN_FILE);
    try {
        if (existsSync(filePath))
