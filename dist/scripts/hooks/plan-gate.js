 ...
 */
export function checkPlanGate(input) {
    // Force bypass
    ...
    for (const p of FILE_PATH_PATTERNS) {
        if (p.test(input))
            anchors.push('file-path');
    }
    for (const p of SYMBOL_PATTERNS) {
        if (p.test(input))
            anchors.push('code-symbol');
    }
    if (ISSUE_REF.test(input))
        anchors.push('issue-ref');
    for (const p of ERROR_PATTERNS) {
        if (p.test(input))
            anchors.push('error-ref');
    }
    if (CODE_BLOCK.test(input))
        anchors.push('code-block');
    if (NUMBERED_LIST.test(input))
        anchors.push('numbered-list');
    if (ACCEPTANCE_CRITERIA.test(input))
        anchors.push('acceptance-criteria');
