...
function findOfficeDir() {
    ...
    if (existsSync(join(direct, 'bin', 'agent-ui.mjs')))
        return direct;
    // 2. Search in ~/Project
            ...
            for (const entry of readdirSync(projectDir)) {
                ...
                if (statSync(candidate).isDirectory()) {
                    if (entry === 'sw-world-agents-view' || entry === 'aing-office') {
                        if (existsSync(join(candidate, 'bin', 'agent-ui.mjs')))
                            return candidate;
                    }
                    // One level deeper
                    try {
                        for (const sub of readdirSync(candidate)) {
                            if (sub === 'sw-world-agents-view' || sub === 'aing-office') {
                                ...
                                if (existsSync(join(subPath, 'bin', 'agent-ui.mjs')))
                                    return subPath;
                            }
                        }
                    }
...
function install() {
    ...
    if (officeDir) {
        ...
        for (const [event, config] of Object.entries(hookScripts)) {
            ...
            const alreadyExists = settings.hooks[event].some((entry) => entry.hooks?.some((h) => h.command?.includes(config.script)));
            if (!alreadyExists) {
                const hookEntry = {
                    hooks: [{
                            type: 'command',
    ...
    if (settings.hooks) {
        for (const event of ['SessionStart', 'PreToolUse', 'PostToolUse']) {
            if (settings.hooks[event]) {
                ...
                settings.hooks[event] = settings.hooks[event].filter((entry) => !entry.hooks?.some((h) => h.command?.includes('session-connect.mjs') ||
                ...
                if (removed > 0)
                    console.log(`OK Hook removed: ${event} (${removed})`);
...
//# sourceMappingURL=agent-ui-setup.js.map