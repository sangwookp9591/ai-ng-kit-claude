...
// ─── Main Simulator ─────────────────────────────────────────────
export function simulateHarness(projectDir) {
    ...
    const errors = steps.filter(s => s.status === 'missing').map(s => `Phase ${s.phase}: ${s.agent}의 ${s.action} "${s.artifact}" — 소스 없음`);
    const warnings = [
        ...steps.filter(s => s.status === 'mismatch').map(s => `Phase ${s.phase}: ${s.agent}의 ${s.action} "${s.artifact}" — ${s.detail}`),
        ...errorScenarios,
    ];
...
function simulateDataFlow(config) {
    const steps = [];
    const agentOutputs = new Map(); // agent → produced artifacts
...
function isArtifactAvailable(artifact, source, outputs) {
    if (source === 'input')
        return true;
...
function groupEdgesByPhase(edges) {
...
function simulateCommunication(config) {
    const graph = [];
    for (const agent of config.agents) {
...
function simulateErrors(config) {
    const warnings = [];
...
// ─── Display ────────────────────────────────────────────────────
export function formatSimulation(result) {
...
//# sourceMappingURL=harness-simulator.js.map